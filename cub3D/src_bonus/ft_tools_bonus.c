...
/*   Created: 2024-12-29 10:17:14 by sternero          #+#    #+#             */
/*   Updated: 2024-12-29 10:17:14 by sternero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_split_line(char ***c, int fd)
{
	...
	if (!line)
	{
		printf(RED"Error reading the line.\n"RESET);
		return (FAILURE);
	}
	line = ft_del_nl(line);
	*c = ft_split(line, ' ');
	if (!c)
	{
		printf(RED"Error in ft_split process.\n"RESET);
		free(line);
		return (FAILURE);
	}
	free(line);
...
void	ft_new_fd(t_data *info, int i, int *fd)
{
	...
	if (j != 0)
	{
		...
		*fd = open(info->file, O_RDONLY);
		if (*fd < 0)
		{
			printf(RED"Errore opening the map file."RESET);
			return ;
		}
		while (k <= i - j)
		{
