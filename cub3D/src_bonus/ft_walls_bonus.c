...
/*   Created: 2024-12-29 10:17:33 by sternero          #+#    #+#             */
/*   Updated: 2024-12-29 10:17:33 by sternero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_get_imgs_info(t_data *info, int *res, int *fd)
{
	if (*fd < 0)
	{
		printf(RED"Error opening the map file."RESET);
		*res = 1;
		return (*res);
	}
...
void	ft_load_door(t_data *info)
{
	...
	{
		info->textures.do_texture = mlx_load_png(info->textures.do_path);
		if (!info->textures.do_texture)
		{
			printf(RED"Failed to load png for door\n"RESET);
			exit(FAILURE);
		}
	}
}

/*	This function loads the texture of the door. It takes a pointer to a t_data
	structure as an argument. It checks if the door flag is set. If the door 
	flag is set, it loads the texture of the door using the mlx_load_png 
	function. If the texture of the door is not loaded correctly, it prints an
	error message and exits the program. This helps to load the texture of the
	door correctly.	
	Indeed, this function could be included in the ft_load_imgs function, but it
	has been separated to facilitate the reading of the code (for those twenty-
	five lines maximum per function).	*/
