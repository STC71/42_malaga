...
/*   Created: 2024-12-29 10:16:10 by sternero          #+#    #+#             */
/*   Updated: 2024-12-29 10:16:10 by sternero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_search_wall(t_data *info, t_ray *ray)
{
	while (info->map.map[ray->map_y][ray->map_x] != '1'
		...
		&& info->map.map[ray->map_y][ray->map_x] != 'C')
	{
...
/*	The function is in charge of finding the impact point of a ray with a wall,
	...
	over the map until it finds a wall or finds a door. In each iteration, it 
	determines in which direction to move (horizontal or vertical) and updates
	the side distances. If the side distance in x is less than the side distance
	in y, it updates the side distance in x and moves in x. If the side distance
	in y is less than the side distance in x, it updates the side distance in y
	and moves in y. When it finds a wall, it calculates the perpendicular
	distance between the player and the wall. If the ray flag is 0, it 
	calculates the perpendicular distance in x. If the ray flag is 1, it 
	calculates the perpendicular distance in y.	*/