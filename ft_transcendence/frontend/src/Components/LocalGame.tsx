...
interface LocalGameProps {
    setLocalGame: Dispatch<SetStateAction<boolean>>
}
...
export default function LocalGame({ setLocalGame }: LocalGameProps) {
    ...
    const [displayState, setDisplayState] = useState<LocalGameState>(gameStateRef.current);
    ...
    const resetBall = () => {
        ...
        state.ball.x = 400;
        state.ball.y = 200;
        state.ball.vx = (Math.random() > 0.5 ? 1 : -1) * 7;  // Hard difficulty
        state.ball.vy = (Math.random() - 0.5) * 10;          // Hard difficulty
        state.ball.speed = 7;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 flex flex-col items-center justify-center p-8">
            <h1 className="text-5xl font-bold text-white mb-4 text-center">
                {getText('localGamePongGame')}
            </h1>
            {displayState.gameStarted && !displayState.gameEnded && (
                <div className="mb-4 flex justify-center items-center">
                    <span className="bg-gradient-to-br from-blue-600 to-blue-950 w-96 h-16 gap-2 flex justify-evenly items-center rounded-2xl text-xl font-semibold text-white overflow-hidden">
                        <h1 className="text-2xl font-bold text-white/90">{getText('localGamePlayer1')}</h1>
                        <div className="gap">
                        ...
                        <h1 className="text-2xl font-bold text-white/90">{getText('localGamePlayer2')}</h1>
                    </span>
                </div>
            )}
            <div className="relative">
                ...
                {!displayState.gameStarted && !displayState.gameEnded && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/70 rounded-lg">
                        <button
                            onClick={startGame}
                            className="px-12 py-6 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 text-white text-3xl font-bold rounded-lg shadow-lg transition-all duration-200 hover:scale-105 active:scale-95"
                        >
                            {getText('localGameStartGame')}
                        </button>
                    </div>
                )}
                ...
                {displayState.gameEnded && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-lg">
                        <h2 className="text-6xl font-bold text-yellow-400 mb-4 animate-pulse">
                            🏆 {displayState.winner === 'Player 1' ? getText('localGamePlayer1') : getText('localGamePlayer2')} {getText('localGameWins')}
                        </h2>
                        <p className="text-2xl text-white mb-8">
                            Score: {displayState.player1.score} - {displayState.player2.score}
                        </p>
                        <button
                            onClick={resetGame}
            ...
            <div className="mt-6 flex gap-4 items-center">
                {showControls && (
                    <div className="bg-gray-800/90 px-8 py-4 rounded-lg border-2 border-gray-600 relative">
                        <button
                        ...
                        <div className="flex gap-12 text-white text-lg">
                            <div className="flex items-center gap-3">
                                <span className="font-bold text-blue-400">{getText('localGamePlayer1')}:</span>
                            ...
                            <div className="flex items-center gap-3">
                                <span className="font-bold text-red-400">{getText('localGamePlayer2')}:</span>
                ...
                <button
                    onClick={() => {soundEnabled.current = !soundEnabled.current;setDisplayState({...gameStateRef.current});}}
                    className={`px-6 py-4 rounded-lg font-bold transition-all duration-200 ${
                        soundEnabled.current
                            ? 'bg-green-600 hover:bg-green-700 text-white'
                            : 'bg-gray-600 hover:bg-gray-700 text-gray-300'
                    }`}
                >
                    {soundEnabled.current ? getText('localGameAudioOn') : getText('localGameAudioOff')}
                </button>
            </div>
            ...
            {displayState.gameStarted && !displayState.gameEnded && (
                <button
                    onClick={resetGame}
                    className="mt-6 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-lg transition-all duration-200"
                >
                    {getText('localGameStopGame')}
                </button>
            )}
